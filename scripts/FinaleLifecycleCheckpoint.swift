...
final class StoreCapture: XCTestCase {
 ...
 func testDarkFinaleBackgroundAndRelaunch(){
  ...
  let complete=NSPredicate(format:"label CONTAINS %@","started paused false remaining 0")
  expectation(for:complete,evaluatedWith:status);waitForExpectations(timeout:45)
