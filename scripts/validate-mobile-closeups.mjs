...
const source=await loadImage('public/'+ATLAS_URLS.micro.slice(2));
const board=createCanvas(1024,560),b=board.getContext('2d');b.fillStyle='#041423';b.fillRect(0,0,1024,560);
const samples=[];
let column=0;
for(const id of ['hunter','giant'])for(const energy of [1,1.5]) {
  ...
  for(const f of [0,Math.floor(m.frames*.27),Math.floor(m.frames*.71)]) {
    ...
    if(f===0){b.drawImage(expected,column*256,30);b.drawImage(actual,column*256,290);b.fillStyle='#fff';b.font='14px sans-serif';b.fillText(id+' '+energy,column*256+12,20);}
  }
  column++;
}
writeFileSync('design/mobile-performance-2026-09-22/closeup-comparison.png',board.toBuffer('image/png'));
writeFileSync('design/mobile-performance-2026-09-22/visual-validation.json',JSON.stringify({kind:'Original rig above, exported animation below; Skia pixels, no iOS GPU measurement',samples},null,2)+'\n');
