...
mkdirSync('design/automatic-benchmark-2026-09-22',{recursive:true});
try {
  ...
  await page.screenshot({path:'design/automatic-benchmark-2026-09-22/completed-webkit.png'});
  ...
  const result={kind:'Desktop WebKit functional test with accelerated rAF clock, NOT performance results',scenarios:report.results.length,
    statuses:report.results.map(x=>({id:x.id,size:x.size,status:x.status})),fileBytes:data.length,filename:download.suggestedFilename(),campaignRestored:true,errors};
  writeFileSync('design/automatic-benchmark-2026-09-22/browser-smoke.json',JSON.stringify(result,null,2)+'\n');
