...
const out='design/menu-theme-selector-2026-09-24';
...
try {
  for (const {name,width,height,language} of [
    ...
    for(const theme of ['current','organic','mixed']){
      ...
      const state=await page.evaluate(()=>{
        ...
        return {theme:menu.dataset.menuTheme,selected:selector.value,saved:localStorage.getItem('voro-menu-theme-v1'),
          background:getComputedStyle(dialog).backgroundImage,frameFilter:getComputedStyle(frame).filter,
    ...
    if(await page.locator('#voro-menu-theme').inputValue()!=='mixed')throw new Error(`${name}: theme not restored`);
  ...
  const current=results.filter(r=>r.theme==='current'),mixed=results.filter(r=>r.theme==='mixed');
  ...
  console.log(JSON.stringify({captures:results.length,restores:4,controlsPreserved:12,journey:4,credits:4,backgroundSame:4}));
