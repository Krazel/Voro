...
const out='design/finale-core-2026-09-23';
...
try{for(const [port,width,height] of [[5208,390,844],[5206,1280,800]]){
 ...
 const frames=[];
 for(const elapsed of [0,8,10.8,12.2,12.6,12.9,13,17.9,26,31]){
  ...
  if(elapsed>=13&&elapsed<=18)assert.equal(result.lit,0,'Pure black interval');
...
await writeFile(`${out}/checks.json`,JSON.stringify(report,null,2));console.log('Mobile/PC absorption, five-second pure black interval, delayed reveal and 32s preview verified.');
